...
st.markdown("""
    <style>
        /* Reduce default font size for all text elements */
        body {
            font-size: 14px !important;
        }
        /* Reduce font size for titles (st.title) */
        h1 {
            font-size: 24px !important;
        }
        /* Reduce font size for headers (st.header) */
        h2 {
            font-size: 20px !important;
        }
        /* Reduce font size for subheaders (st.subheader) */
        h3 {
            font-size: 18px !important;
        }
        /* Reduce font size for regular text (st.write, labels, etc.) */
        p, label, div.stMarkdown, div.stText {
            font-size: 14px !important;
        }
        ...
        /* Reduce font size for form input labels */
        div.stSelectbox label, div.stTextInput label, div.stNumberInput label, div.stTextArea label {
            font-size: 14px !important;
        }
        /* Reduce font size for plot labels (matplotlib) */
        .stPlotlyChart, .matplotlib-container, .plt-text {
...
if st.button("PARTS & INVENTORY MANAGEMENT") or st.session_state.get("form_active", False):
    ...
    with st.form(key="inventory_form"):
        ...
        name_description = st.text_area("NAME AND DESCRIPTION", value=st.session_state.get("name_description", "OIL FILTER - STANDARD SIZE"), key="name_description_input")
        ...
        for idx, tech in enumerate(technicians):
            with st.expander(f"TECHNICIAN {idx + 1} - {tech['NAME']}"):
                ...
                if col2.button("DELETE", key=f"delete_technician_{idx}"):
                    ...
                    conn.commit()
                    conn.close()
                    st.success("TECHNICIAN PROFILE DELETED SUCCESSFULLY!")
                    st.experimental_rerun()
...
if st.button("CALCULATE"):
    ...
    for lambda_val in lambda_values:
        ...
        tau1_s90.append(f'{tau_de[0]:.2f}' if S_de >= S_star_90 else 'NI')
    ...
    for lambda_val in lambda_values:
        ...
        tau1_s85.append(f'{tau_de[0]:.2f}' if S_de >= S_star_85 else 'NI')
...
st.write("LAST UPDATED: WEDNESDAY, JUNE 25, 2025, 07:42 AM -03")
